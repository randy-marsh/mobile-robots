...
% Load a binary occupancy grid of the office environment in Gazebo. The map
% is generated by driving TurtleBot inside the office environment. The map
% is constructed using range-bearing readings from Kinect(R) and
% ground truth poses from |gazebo/model_states| topic.
% Refer to <docid:robotics_examples.example-MappingWithKnownPosesExample Mapping With Known Poses>
% for a more detailed explanation.
%filePath = fullfile(fileparts(which('TurtleBotMonteCarloLocalizationExample')),'data','officemap.mat');
filePath = 'C:\Users\Juan\Documents\MUII\robotizados\practicas\matlabscripts\gmapping\prueba_giroparado.mat';
load(filePath);
show(map);
%%
...
rangeFinderModel.SensorLimits = [0.15 8];
...
%% rangeFinderModel.SensorPose = [8 6 0];
...
% commands.
% [velPub,velMsg] = ...
%     rospublisher('/robot0/cmd_vel','geometry_msgs/Twist');

%% Initialize AMCL Object
% Instantiate an AMCL object |amcl|. See <docid:robotics_ref.bu31hfz-1
% robotics.MonteCarloLocalization> for more information on the class.
...
amcl.ParticleLimits = [1000 5000];
amcl.GlobalLocalization = true;
% amcl.InitialPose = ExampleHelperAMCLGazeboTruePose();
% use initial pose from .launch
initialPose = [11.22 7.72 320];
amcl.InitialPose = initialPose;
amcl.InitialCovariance = eye(3);
...
% plotStep(visualizationHelper, amcl, [8 6 0], scan, 0)
%%
...
while ishandle(h)
    ...
    % added a try catch to prevent crashing
    try
    scan = lidarScan(scanMsg);
    catch
    ...
    disp(pose)
    pose(1) = pose(1) + initialPose(1);
    pose(2) = pose(2) + initialPose(2);
    pose(1) = pose(3) + initialPose(3);
    ...
    %wander(wanderHelper);
    % move it with teleop_twist_keyboard.py
